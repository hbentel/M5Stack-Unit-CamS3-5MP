...
/* SOC_GDMA_PAIRS_PER_GROUP removed in IDF v6; replaced by GDMA_LL_AHB_PAIRS_PER_GROUP */
#define SOC_GDMA_PAIRS_PER_GROUP GDMA_LL_AHB_PAIRS_PER_GROUP
#else
...
static void IRAM_ATTR ll_cam_vsync_isr(void *arg)
{
    ...
    if (!status.cam_vsync_int_st) return;

    cam->debug.vsync_isr_count++;
    cam->isr_seq++;

    if (cam->psram_mode && cam->jpeg_mode) {
        // === Autonomous ISR for PSRAM JPEG (multi-buffer) ===
        // Matches ll_cam_prepare_frame() register sequence exactly.

        // --- 1) Stop capture + snapshot EOF count ---
        uint8_t finished_pos = cam->cur_frame_pos;
        uint32_t finished_cnt = cam->cur_eof_cnt;

        if (cam->capturing) {
            LCD_CAM.cam_ctrl1.cam_start = 0;
            // Do NOT use link.stop — legacy path doesn't use it, and it may
            // put GDMA in a state that in_rst + link.start can't recover from.
            cam->capturing = false;
        }

        // Send frame-done to task, OR return buffer to free pool
        if (finished_cnt > 0) {
            cam_frame_done_t done = { finished_pos, finished_cnt, cam->isr_seq };
            if (xQueueSendFromISR(cam->frame_done_queue, &done, &hp) != pdTRUE) {
                // Queue full — return buffer to free pool to prevent leak
                __atomic_fetch_or(&cam->free_mask, (1u << finished_pos), __ATOMIC_SEQ_CST);
                cam->debug.queue_overflow_count++;
            }
        } else if (cam->isr_seq > 1) {
            // No data captured (eof_cnt==0) — return buffer to free pool
            // (skip on very first VSYNC where no buffer was claimed yet)
            __atomic_fetch_or(&cam->free_mask, (1u << finished_pos), __ATOMIC_SEQ_CST);
        }

        // --- 2) Find next free buffer ---
        uint32_t free = cam->free_mask;
        if (free == 0) {
            cam->drops_no_free_buf++;
            if (hp) portYIELD_FROM_ISR();
            return;  // No free buffer — capture stalls until cam_give() frees one
        }

        uint8_t next = __builtin_ctz(free);  // lowest free bit

        // Claim buffer atomically (not free anymore — ISR owns it for capture)
        __atomic_fetch_and(&cam->free_mask, ~(1u << next), __ATOMIC_SEQ_CST);
        cam->cur_frame_pos = next;
        cam->cur_eof_cnt = 0;

        // --- 3) Start next capture ---
        // Perform the same reset sequence as ll_cam_prepare_frame() to ensure clean
        // alignment each frame. cam_reset + cam_afifo_reset flush the LCD_CAM state
        // machine and async FIFO; in_rst + peri_sel reset and re-connect the GDMA channel.
        // At XCLK=10MHz the front porch gives ~3μs budget; these ~10 register writes take
        // ~100ns total — well within budget. Without these resets, cam_start toggling can
        // produce a spurious extra VSYNC interrupt that cuts the next frame short.
        ...
        GDMA.channel[cam->dma_num].in.peri_sel.sel = 5;  // 5 = LCD_CAM (in_rst resets this)

        // Re-apply burst mode (in_rst clears these bits)
        GDMA.channel[cam->dma_num].in.conf0.indscr_burst_en = 1;
        GDMA.channel[cam->dma_num].in.conf0.in_data_burst_en = 1;

        // Repoint GDMA to the next buffer's descriptor chain
        GDMA.channel[cam->dma_num].in.link.addr = ((uint32_t)&cam->frames[next].dma[0]);
        GDMA.channel[cam->dma_num].in.link.stop = 0;
        GDMA.channel[cam->dma_num].in.link.start = 1;

        LCD_CAM.cam_ctrl1.cam_rec_data_bytelen = cam->dma_half_buffer_size - 1;
        LCD_CAM.cam_ctrl.cam_update = 1;
        LCD_CAM.cam_ctrl1.cam_start = 1;

        cam->capturing = true;
        cam->debug.start_trigger_count++;
    } else {
        // === Legacy path: send VSYNC event to task ===
...
static void IRAM_ATTR ll_cam_dma_isr(void *arg)
{
    ...
    if (status.in_suc_eof) {
        ...
        if (cam->psram_mode && cam->jpeg_mode) {
            cam->cur_eof_cnt++;   // ISR-side count only, no queue event
        } else {
...
bool ll_cam_prepare_frame(cam_obj_t *cam, int frame_pos)
{
    ...
    // Reset GDMA FIFO (Critical for alignment!)
    GDMA.channel[cam->dma_num].in.conf0.in_rst = 1;
    GDMA.channel[cam->dma_num].in.conf0.in_rst = 0;
    
    // Re-apply peri_sel for safety (in_rst resets it)
    GDMA.channel[cam->dma_num].in.peri_sel.sel = 5;  // 5 = LCD_CAM

    LCD_CAM.cam_ctrl1.cam_rec_data_bytelen = cam->dma_half_buffer_size - 1; // Ping pong operation

    if (!cam->psram_mode) {
        GDMA.channel[cam->dma_num].in.link.addr = ((uint32_t)&cam->dma[0]);
    } else {
        GDMA.channel[cam->dma_num].in.link.addr = ((uint32_t)&cam->frames[frame_pos].dma[0]);
    }
...
static esp_err_t ll_cam_dma_init(cam_obj_t *cam)
{
    ...
    // Enable burst mode for both internal SRAM and PSRAM (item 3)
    GDMA.channel[cam->dma_num].in.conf0.indscr_burst_en = 1;
    GDMA.channel[cam->dma_num].in.conf0.in_data_burst_en = 1;

    GDMA.channel[cam->dma_num].in.conf1.in_check_owner = 0;
    // 64-byte PSRAM burst granularity — matches ll_cam_get_dma_align() (64 bytes) and
    // ESP32-S3 D-cache line size. Buffers are already 64-byte aligned and
    // dma_half_buffer_size (2048) is a multiple of 64, so the precondition is satisfied.
    // Default (register reset value) is 0 = 16-byte bursts, which wastes 4x bus transactions.
    // Soak-tested 30 min (no_soi=0, no_eoi=0, no drops, no crashes).
    GDMA.channel[cam->dma_num].in.conf1.in_ext_mem_bk_size = 2;

    GDMA.channel[cam->dma_num].in.peri_sel.sel = 5;
    ESP_LOGI(TAG, "GDMA ch%d init: peri_sel=%lu conf0=0x%08lx conf1=0x%08lx link=0x%08lx",
        cam->dma_num,
        (unsigned long)GDMA.channel[cam->dma_num].in.peri_sel.sel,
        (unsigned long)GDMA.channel[cam->dma_num].in.conf0.val,
        (unsigned long)GDMA.channel[cam->dma_num].in.conf1.val,
        (unsigned long)GDMA.channel[cam->dma_num].in.link.val);
    return ESP_OK;
}
...
esp_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    // VSYNC glitch filter: reject pulses shorter than 4 LCD_CAM clock cycles.
    // At 160 MHz that is 4 × 6.25 ns = 25 ns — long enough to suppress RF/Wi-Fi
    // induced noise on the VSYNC line while still passing the PY260's real VSYNC
    // pulse (which is many microseconds wide).
    ...
    LCD_CAM.cam_ctrl.cam_update = 1;
    LCD_CAM.cam_ctrl1.cam_start = 1;   // Peripheral must be running to detect VSYNC edges
...
{
    return 64; // Force 64-byte cache line alignment for S3
}
...
bool ll_cam_dma_sizes(cam_obj_t *cam)
{
    ...
    if (cam->jpeg_mode) {
        if (cam->psram_mode) {
            ...
            cam->dma_half_buffer_size = 2048; // Hardware max: lldesc_t.size is 12-bit (max 4095), LCD_CAM_DMA_NODE_BUFFER_MAX_SIZE=2048
            cam->dma_half_buffer_cnt = cam->dma_buffer_size / cam->dma_half_buffer_size;
